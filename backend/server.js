...
// CORS for split deployment (frontend on Vercel, backend on Render)
...
if (process.env.SERVE_STATIC !== 'false') {
    ...
    app.get('/master-control', requireAuth('super_admin'), (req, res) => {
        res.sendFile(path.join(publicDir, 'html', 'master-control.html'));
    });

    app.get('/branch-admin', requireAnyRole(['super_admin', 'branch_admin']), (req, res) => {
        res.sendFile(path.join(publicDir, 'html', 'branch-admin.html'));
    });
